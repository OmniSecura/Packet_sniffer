...
static QByteArray followStreamPcapFixture()
{
    static const unsigned char data[] = {
        ...
        
    };

    return QByteArray::fromRawData(reinterpret_cast<const char*>(data), sizeof(data));
}
...
void SniffingTest::reconstructTcpStreamFromPcap()
{
    Sniffing sniffer;
    sniffer.resetStreams();

    QTemporaryFile pcapFile;
    QVERIFY2(pcapFile.open(), "Failed to create temporary pcap fixture");
    const QByteArray fixture = followStreamPcapFixture();
    QVERIFY2(pcapFile.write(fixture) == fixture.size(), "Failed to populate pcap fixture");
    QVERIFY(pcapFile.flush());

    sniffer.openFromPcap(pcapFile.fileName());
    const auto streams = sniffer.getStreamConversations();
    QCOMPARE(streams.size(), 1);

    const auto &stream = streams.first();
    QCOMPARE(int(stream.protocol), int(IPPROTO_TCP));
    QCOMPARE(stream.protocolName(), QStringLiteral("TCP"));
    QCOMPARE(stream.endpointA.address, QStringLiteral("192.0.2.10"));
    QCOMPARE(stream.endpointA.port, quint16(12345));
    QCOMPARE(stream.endpointB.address, QStringLiteral("198.51.100.25"));
    QCOMPARE(stream.endpointB.port, quint16(80));
    QCOMPARE(stream.packetCount, 2);
    QCOMPARE(stream.segments.size(), 2);
    QVERIFY(stream.initiatorIsA);

    const QByteArray expectedRequest("GET / HTTP/1.1\r\nHost: example.com\r\n\r\n");
    const QByteArray expectedResponse(
        "HTTP/1.1 200 OK\r\n"
        "Content-Length: 5\r\n"
        "Connection: close\r\n\r\nHello");

    QCOMPARE(stream.aggregatedAToB, expectedRequest);
    QCOMPARE(stream.aggregatedBToA, expectedResponse);
    QCOMPARE(stream.totalBytesAToB, qint64(expectedRequest.size()));
    QCOMPARE(stream.totalBytesBToA, qint64(expectedResponse.size()));

    const auto &requestSegment = stream.segments.at(0);
    QVERIFY(requestSegment.fromAtoB);
    QCOMPARE(requestSegment.payload, expectedRequest);
    QCOMPARE(requestSegment.payloadLength, expectedRequest.size());
    QCOMPARE(requestSegment.sequenceNumber, quint32(1));
    QCOMPARE(requestSegment.acknowledgementNumber, quint32(0));

    const auto &responseSegment = stream.segments.at(1);
    QVERIFY(!responseSegment.fromAtoB);
    QCOMPARE(responseSegment.payload, expectedResponse);
    QCOMPARE(responseSegment.payloadLength, expectedResponse.size());
    QCOMPARE(responseSegment.sequenceNumber, quint32(5001));
    QCOMPARE(responseSegment.acknowledgementNumber, quint32(expectedRequest.size() + 1));
}
